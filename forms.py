...
class RegistrationForm(FlaskForm):
    ...
    email = StringField('Email', validators= [InputRequired(), DataRequired(), Email(), Regexp(regex="^[a-zA-Z]+[.][a-zA-Z]+[0-9]*@?(amdocs)\\.com$", message="Email should be of the format 'name.lastname@amdocs.com'")])
    address = StringField('Address',
                          validators=[DataRequired()])
    mobile_number = StringField('Mobile Number',
                                 validators=[DataRequired(), Length(min=10, max=10), Regexp('\d{10}', message='Only digits allowed.')])
